...
namespace SimpleLauncher.Application.Services
{
    public class ServerListService : IServerListService
    {
        ...
        public async Task<List<PlayerMeta>?> GetServerPlayersFromMonitoringAsync(string ipAddressAndPort, 
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var ipAndPortArray = ipAddressAndPort.Split(":");
            if (ipAndPortArray.Length != 2 ||
                !ushort.TryParse(ipAndPortArray[1], out var port))
                return null;
            return await GetServerPlayersFromMonitoringAsync(ipAndPortArray[0], port, cancellationToken);
        }
        public async Task<List<PlayerMeta>?> GetServerPlayersFromMonitoringAsync(string ipAddress, 
            ushort port, 
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IpAndPortValidator(ipAddress, port))
                return null;
            return await _monitoringApiGateway.GetServerPlayers(ipAddress, port, cancellationToken);
        }
